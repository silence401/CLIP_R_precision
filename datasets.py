...
class CustomDataset(BaseDataset):
    """AI is creating summary for CustomDataset

    Args:
        BaseDataset ([type]): [description]
    """
    def __init__(self, data_dir=None, dataset_name=None, comp_type=None, split=None, image_transform=None, tokenizer=None):
        super().__init__(data_dir, dataset_name, comp_type, split, image_transform, tokenizer)
        self.caption_list = []
        self.image_list = []
        self.mismatch_texts = []
        ...
        for i in range(len(cis)):
            ...
            if self.tokenizer is not None:
                self.caption_list.append(self.tokenizer(caption))
            else:
                self.caption_list.append(caption)

    def init_mismatch_texts(self, data_dir=None):
        """AI is creating summary for init_mismatch_texts
        """
    ...
    def __getitem__(self, index):
        image_path = self.caption_list[index], 
        caption = self.image_list[index]
        if self.tokenizer is not None:
